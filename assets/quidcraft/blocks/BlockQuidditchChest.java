...
public class BlockQuidditchChest extends Block {
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int f, float f1, float f2, float f3) {
		...
		/*
		 * for(int c=0;c<blocks.length;c++){ outputPitch += " " + blocks[c];
		...
		/*
		 * Writer output = null; File file = new File("pitchfile.txt"); try {
		 * output = new BufferedWriter(new FileWriter(file));
		 * output.write(outputPitch); output.close(); } catch (IOException e) {
		 * e.printStackTrace(); }
		 */
		//}
		if (l == 0) {
	...
	/*
	 * public int idDropped(int i, Random random,int j){ return blockID; }
	 */
	@Override
	@SideOnly(Side.CLIENT)
