...
public class BlockQuidditchChest extends Block{
	protected BlockQuidditchChest(int i, int j)
    {
    ...
    {
        if(i == 0 || i == 1)//bottom, top
        {
            return 20;//chestTop;
        }
        else//(i == 2 || i == 3 || i == 4)//sides
        {
            return 21;//chestSide;
        }
    }
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int f, float f1,float f2,float f3)
    {
		...
		//String pitchFile = "pitchfile.txt";
		//File pitchFile = new File("pitchfile.txt");
		
		//Scanner scanner = new Scanner(pitchFile);
	...
	public void fillWithBlocks(World world,int i1, int i2, int j1, int j2, int k1, int k2, int material){
		for(int i = i1; i <= i2; i++){
			for(int j = j1; j <= j2; j++){
				for(int k = k1; k <= k2; k++){
					world.setBlockWithNotify(i, j, k, material);
				}
			}
		}
	}
